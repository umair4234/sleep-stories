...
interface ContentSuiteViewProps {
...
const ProjectCard: React.FC<{ project: ScriptProject; onView: () => void; onUpdateStatus: (status: ProjectStatus) => void; onDelete: () => void; }> = ({ project, onView, onUpdateStatus, onDelete }) => {
    ...
    const buttonText: Record<ProjectStatus, string> = {
        working: 'View Progress',
        paused: 'Resume Project',
        failed: 'View & Retry',
        completed: 'View Project',
        uploaded: 'View Project',
    };
    ...
    return (
        <div className="bg-brand-bg p-4 rounded-lg border border-brand-secondary/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0 w-full sm:w-auto">
                {project.status === 'completed' && <button onClick={() => onUpdateStatus('uploaded')} className="text-xs py-1 px-2 rounded bg-blue-500/20 hover:bg-blue-500/40 text-blue-300 transition-colors">Mark as Uploaded</button>}
                {project.status === 'uploaded' && <button onClick={() => onUpdateStatus('completed')} className="text-xs py-1 px-2 rounded bg-green-500/20 hover:bg-green-500/40 text-green-300 transition-colors">Move to Library</button>}
                ...
                <button onClick={onView} className="py-2 px-4 rounded-lg bg-brand-accent text-brand-bg font-bold text-sm hover:bg-yellow-500 transition-colors">
                    {buttonText[project.status] || 'View Project'}
                </button>
            </div>
        </div>
    );
...
type LibraryTab = 'inProgress' | 'library' | 'uploaded';

export const ContentSuiteView: React.FC<ContentSuiteViewProps> = ({ projects, onView, onUpdateStatus, onDelete }) => {
    ...
    const filteredProjects = projects
      .filter(p => {
        switch (activeTab) {
          case 'inProgress':
            return ['working', 'paused', 'failed'].includes(p.status);
          case 'library':
    ...
    const tabButtonClasses = (tabName: LibraryTab) => {
    ...
    return (
        <div className="w-full">
            <div className="border-b border-brand-secondary/20 mb-6">
                <button onClick={() => setActiveTab('inProgress')} className={tabButtonClasses('inProgress')}>In Progress</button>
                <button onClick={() => setActiveTab('library')} className={tabButtonClasses('library')}>Library</button>
            ...
            {filteredProjects.length > 0 ? (
                ...
                <div className="text-center py-16 text-brand-secondary">
                    <h3 className="text-xl font-semibold text-brand-text">No scripts in this view.</h3>
                    <p>Your projects will appear here based on their status.</p>
                </div>
            )}
        </div>
    );
